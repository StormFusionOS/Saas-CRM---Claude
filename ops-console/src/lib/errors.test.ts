...
describe("Error Handling", () => {
  describe("getErrorMessage", () => {
    ...
    it("should return correct message for not found errors", () => {
      const message = getErrorMessage(ErrorCode.ERR_SERVICE_NOT_FOUND);
      expect(message).toBe("Service not found. It may have been removed or renamed.");
    });

    it("should return correct message for validation errors", () => {
      const message = getErrorMessage(ErrorCode.ERR_INVALID_TIME_RANGE);
      expect(message).toBe("Invalid time range selected. Please choose a valid date range.");
    });
